...
- FYI: `integration_tests/scripts/run_interrupt.py` now defaults ZFS configs to the first configured source unless you pass `--source` (or `--source all`), which keeps interrupt/retry runs deterministic for multi-dataset ZFS harness configs.
- FYI: `integration_tests/scripts/run_all.py` now drives the ZFS application-backed path (`setup -> seed -> full -> mutate -> incremental -> interrupt -> restore --source all -> verify_restore --source all -> teardown`); standalone ZFS probes remain manual diagnostics, and `--skip-s3` trims the ZFS run to setup/seed/mutate/teardown.

...
def main() -> int:
    ...
    with open_log(log_path) as log:
        ...
        for obj in objects:
            key = obj["Key"]
            role, layout_error = _classify_key(key, prefix, sources)
            if layout_error:
...
def _classify_key(
    ...
    source, suffix = _split_source_key(remainder, sources)
    if source is None or suffix is None:
        if not remainder.startswith("subvol/"):
            return None, "missing subvol prefix"
        return None, "unexpected source identifier"
    if suffix == "current.json":
        return "current", None
    parts = suffix.split("/")
    if parts[0] not in ("full", "incremental"):
...
def _split_source_key(
    ...
    if not remainder.startswith("subvol/"):
        return None, None
    body = remainder[len("subvol/") :]
    for source in sorted(sources, key=len, reverse=True):
        prefix = f"{source}/"
        if body.startswith(prefix):
            return source, body[len(prefix) :]
    return None, None

"""Seed deterministic data into the configured fixture sources."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed deterministic harness data.")
    ...
    with open_log(log_path) as log:
        ...
        for source in sources:
            ...
            if not os.path.isdir(source_path):
                ...
                return 1
            created = _seed_subvolume(
                source_path,
                source.identifier,
                dataset_size_bytes,
            )
            log.write(f"seeded {source.identifier}: {created}")
